    ...
    async redirectAfterWizard() {
      ...
      switch (wizardType) {
        ...
        default://Local
          ...
          const loadingModal = this.$alert.info({
            mode: 'modal',
            title: this.siteName,
          ...
          let response = null
          try {
            response = await this.$crud.create('apiRoutes.qplan.buy', params)
          } catch (error) {
            this.$alert.error({message: `${this.$tr('isite.cms.message.recordNoCreated')}`})
          }
          //Close the loading modal, whatever the outcome was
          loadingModal.hide()
          if (response?.data?.redirectTo) {
            url = response.data.redirectTo
          } else if (response) {
            //Organization was created but the API returned no redirect URL, fallback to the organizations list
            this.$router.push('/site/my-organizations')
          }
          break
      }
      //Redirect
