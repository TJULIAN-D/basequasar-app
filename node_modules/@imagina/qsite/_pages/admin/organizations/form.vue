    ...
    getSubscriptionData(refresh = false, userId) {
      return new Promise((resolve, reject) => {
        ...
        this.$crud.show('apiRoutes.qplan.subscriptions', userId, requestParams).then(response => {
          ...
          //Organization owner has no subscription assigned, not a real error
          //Note: $crud.show() already unwraps to error.response before rejecting, so status is top-level here
          if (error?.status === 404) {
            this.$alert.info({message: this.$tr('isite.cms.message.organizationWithoutPlan')})
            return resolve(null)
          }
          this.$apiResponse.handleError(error, () => {
            reject(error)
