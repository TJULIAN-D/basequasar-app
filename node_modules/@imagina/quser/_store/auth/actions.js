    ...
    crud.post('apiRoutes.quser.authLogin', dataRequest).then(async response => {
      ...
      try {
        if(response.data.userData){
          ...
          getTokenFirebase(response.data.userData.id, true);//Get firebase token for notifications and update in backend
        }
      } catch (e) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (data) {
        ...
        /*
        try { 
          await getTokenFirebase(data.userData.id, false);//Get firebase token for notifications        
        } catch (e) {
          console.error('Error getting firebase token: ', e)
        } 
          */
